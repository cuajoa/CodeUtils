exec sp_CreateProcedure 'dbo.spSL_GetLiquidaciones'
GO

ALTER PROCEDURE dbo.spSL_GetLiquidaciones	
    @FechaDesde							Fecha,
    @FechaHasta							Fecha,
	@IDFondo							CodigoInterfaz,
    @IDTipoValorCuotaParte				CodigoInterfaz,
	@TimeStamp							TimeStamp,
	...
	@CodInterfazCuotapartistaSetting	DescripcionExtraLarga = NULL
...
--	IDCuotapartista o Num Cuotapartista: @CodInterfazCuotapartista o @NumeroCuotapartista Es el filtro del Request y debería filtrar la consulta final siempre y cuando esté en el universo antes filtrado.
	...
	BEGIN
		...
		BEGIN
			...
			SELECT CodCuotapartista, CodTpCuotapartista
			FROM CUOTAPARTISTAS
			WHERE  CUOTAPARTISTAS.CodInterfaz IN (SELECT CAST(Valor AS varchar(30)) FROM fn_Split(@CodInterfazCuotapartistaSetting, ','))
	...
	BEGIN	
		SELECT @CodCuotapartistaRequest= CodCuotapartista  
		FROM CUOTAPARTISTAS
		WHERE  CUOTAPARTISTAS.NumCuotapartista IN (SELECT CAST(Valor AS varchar(30)) FROM fn_Split(@NumeroCuotapartista, ','))
	...
	IF @IDFondo IS NOT NULL 
		SET @CodFondo = (SELECT CodFondo FROM FONDOSREAL WHERE UPPER(CodInterfaz) = UPPER(@IDFondo))
